...
### 2.3 地点↔站点步行耗时（v0.24.0：仅存实测值，由 `db:walktimes` 重灌）

> 手工估算值已全部剔除（原 10 行 `source='manual'` 与 `plan_legs.minutes` 一并清零）。
> 取值口径见 `scripts/rebuild-walk-times.ts`：起点步行 = `wait_start(首站) − depart(起点)`；
> 到点步行 = `arrive(终点) − alight(末站)`；区间内 `pause→resume` 扣除；保留 0.3~20 分钟。
> 澳科大 B/C、N/O、R 三座严格分为独立目的地（`zone` 列），不混算。
