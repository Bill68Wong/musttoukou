/**
 * GET/POST /api/cron/rebuild —— 派生数据定时重算（v0.25.0）
 *
 * 背景：segment_stats（站间时长）与 walk_times（步行时长）是从原始样本
 * （timer_events / free_ride_events）算出的**派生表**，此前只能手动跑
 * `npm run db:segments` / `npm run db:walktimes` 才会更新 —— 新样本进来后
 ...
 *   并按顺序串行两个重算（都读 timer_events，串行避免连接竞争）。
 *
 * 幂等：可重复调用。
 */
...
async function handle(req: NextRequest) {
  ...
  try {
    ...
    // 顺序执行：两者都读 timer_events，串行可避免 DB 连接竞争
    results.push(
