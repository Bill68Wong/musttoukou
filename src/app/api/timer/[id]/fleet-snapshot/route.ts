 ...
 *
 * 反事实车队快照（需求 9 / v0.11.0 B1）：打点 depart/wait_start/alight 成功后由前端触发（不阻塞打点）。
 * 服务端自解析（不再接收/依赖客户端候选集）：
 *   候选集 = 同 from_place/to_place 且 is_active 的方案 → 每张卡首个巴士段主线路 + 各自上车站
 *   参照站 = 每个候选方案自己的上车站（跨站门到门对比预采，取代历史 compare_routes）
...
export async function POST(req: NextRequest, ctx: { params: Promise<{ id: string }> }) {
  try {
    ...
    const result = await captureFleetSnapshot({ sessionId, stage });
